...
public sealed class BridgeHttpServer : IDisposable
{
    ...
    private Task? worker;
    public BridgeHttpServer(int port, string token, MainThreadQueue queue, bool enableValidation = false, bool enablePlacement = false, bool enableLodgePlacement = false, bool enableSpeedControl = false)
    {
        ...
        this.enableSpeedControl = enableSpeedControl; this.enablePlacement = enablePlacement; this.enableLodgePlacement = enableLodgePlacement;
    ...
    private async Task Handle(HttpListenerContext context)
    {
        ...
        else if (!MethodAllowed(request.HttpMethod, request.Url!.AbsolutePath, request.HasEntityBody, enableValidation, enablePlacement, enableLodgePlacement, enableSpeedControl))
    ...
    public static bool MethodAllowed(string method, string path, bool hasBody, bool enableValidation, bool enablePlacement = false, bool enableLodgePlacement = false, bool enableSpeedControl = false) =>
        !hasBody && (path == "/agent-api/v1/simulation-speed" ? enableSpeedControl && method == "POST" : path == "/agent-api/v1/lodge-placement" ? enableLodgePlacement && method == "POST" : path == "/agent-api/v1/path-placement" ? enablePlacement && method == "POST"
