...
public sealed partial class NativeClient
{
    ...
    {
        if(e.BridgeVersion!="0.19.0"||r.Session.Length>0&&e.SessionId!=r.Session||limits is null||limits.Any(s=>s is null))throw new InvalidDataException("Invalid logistics envelope");
    }
    ...
    public async Task<BridgeEnvelope<NativeRoad>> RoadConnection(LogisticsRequest r,CancellationToken ct)
    {
        ...
        if(d.Id!=r.Id||d.ToId!=r.ToId||d.SourceAccessCount<0||d.TargetAccessCount<0||d.Supported!=(d.Connected is not null)||
    ...
    public async Task<BridgeEnvelope<NativeRange>> WorkRange(LogisticsRequest r,CancellationToken ct)
    {
        ...
        if(d.Id!=r.Id||d.Items is null||!Page(r,d.Offset,d.Limit,d.Total,d.Items.Length,d.HasMore)||d.Total>65536||d.RangeNames is null||d.RangeNames.Length>16||d.RangeNames.Any(n=>n is null||n.Length>160)||
