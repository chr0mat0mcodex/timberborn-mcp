...
public sealed partial class NativeClient
{
    ...
    public async Task<BridgeEnvelope<NativeAreas>> Areas(ManagementRequest r,CancellationToken ct)
    {
        ...
        if(r.Kind=="tapping"&&(result.BridgeVersion is not ("0.13.2" or "0.14.0" or "0.14.1" or "0.15.0" or "0.16.0" or "0.17.0")||d.Items.Any(c=>c.Vegetation?.IsTappingCandidate()!=true)))throw new InvalidDataException("Tapping health unavailable or ineligible");
        return result;
    }
