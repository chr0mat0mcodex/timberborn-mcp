...
public sealed partial class NativeClient
{
    public async Task<BridgeEnvelope<NativeResearch>> Research(ResearchRequest r,CancellationToken ct)
    {
        ...
        if(e.BridgeVersion is not ("0.17.0" or "0.17.1" or "0.17.2" or "0.18.0")||d.SciencePoints<0||d.Offset!=r.Offset||d.Limit!=r.Limit||d.Total<0||d.Items is null||
    ...
    public async Task<BridgeEnvelope<NativeUnlock>> UnlockBuilding(ResearchRequest r,CancellationToken ct)
    {
        ...
        var d=e.Data;
        if(e.BridgeVersion is not ("0.17.0" or "0.17.1" or "0.17.2" or "0.18.0")||e.SessionId!=r.Session||d.Template!=r.Template||d.ExpectedCost!=r.ExpectedCost||d.ScienceCost<0||d.PointsBefore<0||d.PointsAfter<0||
