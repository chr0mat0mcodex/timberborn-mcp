...
public sealed partial class NativeClient : IDisposable
{
    ...
    public async Task<BridgeEnvelope<NativeBuilding>> Building(BridgeRequest r, CancellationToken ct)
    {
        ...
        if (d.Details is { } b)
        {
            if (result.BridgeVersion is "0.9.0" or "0.10.0" or "0.11.0" or "0.12.0" or "0.13.0" && b.Operations is null)
    ...
    private async Task<BridgeEnvelope<T>> Get<T>(string route, CancellationToken ct, HttpMethod? method = null)
    {
        ...
        if (result is null || result.SchemaVersion != 1 || result.BridgeVersion is not ("0.2.0" or "0.3.0" or "0.4.0" or "0.4.1" or "0.5.0" or "0.6.0" or "0.6.1" or "0.7.0" or "0.8.0" or "0.9.0" or "0.10.0" or "0.11.0" or "0.12.0" or "0.13.0") ||
