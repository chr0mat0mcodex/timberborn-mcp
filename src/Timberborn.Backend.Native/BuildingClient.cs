...
public sealed partial class NativeClient
{
    public async Task<BridgeEnvelope<NativeBuildingCatalog>> BuildingCatalog(BridgeRequest r, CancellationToken ct)
    {
        ...
        var d = result.Data;
        if (result.BridgeVersion is not ("0.14.0" or "0.14.1" or "0.15.0" or "0.16.0" or "0.17.0" or "0.17.1") || string.IsNullOrEmpty(d.Faction) || d.Offset != r.Offset || d.Limit != r.Limit ||
