...
public sealed partial class NativeClient
{
    public async Task<BridgeEnvelope<NativeRemovalTargets>> RemovalTargets(RemovalRequest r,CancellationToken ct)
    {
        ...
        if(d.Items.Any(i=>(i.Vegetation is not null&&!i.Vegetation.IsValid())||(result.BridgeVersion is "0.13.2" or "0.14.0" or "0.14.1" or "0.15.0" or "0.16.0" or "0.17.0"&&(i.Kind is "planted" or "vegetation")&&i.Vegetation is null)))throw new InvalidDataException("Missing or invalid vegetation state");
        return result;
    }
