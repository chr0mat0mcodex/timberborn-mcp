...
public sealed record NativeNeeds(string Scope,int Beavers,int ObservedBeavers,int MissingNeedManagers,int Offset,int Limit,int Total,NeedSummary[] Items,bool HasMore,string[] Limitations,int DeadExcluded,int UnknownLifeStateExcluded);
...
public sealed partial class NativeClient
{
    ...
    public async Task<BridgeEnvelope<NativeNeeds>> Needs(DiagnosticsRequest r,CancellationToken ct)
    {
        ...
        if(d.Scope!="living_beavers_with_need_manager"||d.DeadExcluded<0||d.UnknownLifeStateExcluded<0||d.Beavers<0||d.ObservedBeavers<0||d.MissingNeedManagers<0||(long)d.ObservedBeavers+d.MissingNeedManagers!=d.Beavers||d.Items is null||
    ...
    public async Task<BridgeEnvelope<NativeBeaverNeeds>> BeaverNeeds(DiagnosticsRequest r,CancellationToken ct)
    {
        ...
        if(d.LifeState is not ("alive" or "dead" or "unknown")||d.LifeState!="alive"&&d.Supported||d.Id!=r.Id||d.WorldPosition is null||!float.IsFinite(d.WorldPosition.X)||!float.IsFinite(d.WorldPosition.Y)||!float.IsFinite(d.WorldPosition.Z)||d.Items is null||
