...
public static class ManagementTools
{
    ...
    public static IEnumerable<Tool> Catalog(bool priorities,bool areas)
    {
        ...
        foreach(var (name,route) in Routes){
            ...
            if(route=="set-area"){
                Str("kind","tree_cutting","crops","tree_planting");Str("operation","mark","remove");Str("resource");Str("expectedResource");Str("session");
            ...
            string description=route switch{
                ...
                "area-types"=>"Liest unterstützte Flächenarten und Pflanzvorlagen mit regulärem Werkzeug-Lockstatus. Baumfällen und Pflanzmarkierungen; tapping ist bisher keine belegte eigenständige Markierungsart. Kein Unlock.",
                "areas"=>"Liest markierte Zellen kolonieweit, gefiltert nach Flächenart, maximal 32 pro Seite. crops/tree_planting sind Pflanzaufträge, keine frei definierten Erntezonen. tapping liefert supported=false und total=null, nicht behauptete leere Fläche. Jede Seite frisch.",
                _=>"Markiert/entfernt Markierungen in maximal 4x4 Zellen auf einer Z-Ebene. Vorher inspect_area_types/inspect_areas lesen. Alle Zellen müssen expectedResource entsprechen: unmarked, für Baumfällen marked, sonst Vorlagenname. resource bei Baumfällen/Entfernen leer, sonst freigeschaltete Pflanzvorlage. Geometrie und Pflanzenvalidierung vor Mutation. Entfernt nur Markierung, niemals Pflanzen. Teiländerung möglich; kein Retry/Rollback; Ergebnis nachlesen. tapping nicht unterstützt."};
