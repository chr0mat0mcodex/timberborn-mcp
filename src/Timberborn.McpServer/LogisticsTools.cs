    ...
    public static IEnumerable<Tool> Catalog()
    {
        ...
        foreach(var (name,route) in Routes){
            ...
            yield return new Tool{Name=name,Description=(route switch{
                ...
                "road-connection"=>"Prüft ab 0.19.1 die öffentliche Sofort-Spiel-Wegverbindung vom Gebäude id nach toId. Nur fertige Objekte mit jeweils genau einem gültigen Accessible werden unterstützt. supported=false ist unbekannt, nicht unerreichbar. connected=false ist eine abgelehnte aktuelle Wegsuche. Distanz ist Spielmetrik, keine Reisezeit. Kein Terrain-Abkürzungsweg, keine Bauvorschau.",
                "work-range"=>"Liest ab 0.19.2 die native BuildingTerrainRange eines fertigen Gebäudes; alternativ vorhandene öffentliche Range-Provider. source nennt die belegte Quelle. Maximal 32 pro Seite, Z/Y/X-Sortierung. supported=false bedeutet keine belegte Reichweite; leere Liste nicht als Reichweite null auslegen. Reichweite beweist weder passenden Rohstoff noch aktuelle Arbeitszuweisung. Alle Seiten sind getrennte Beobachtungen.",
                _=>"Liest gespeicherte globale Güterstatistik des Spiels in dessen Reihenfolge: Cycle, Day, Stock, Capacity, Production und Consumption. Produktions-/Verbrauchszähler stammen aus dem Spiel, nicht aus Vorratsdifferenzen. netProduction=Production-Consumption; stockChange separat. Summen gelten nur für die gelesene Seite. Keine angenommene Abtastdauer oder momentane Rate; leere Historie bedeutet fehlende Daten. Maximal 32 Einträge pro Seite."})+" Ab Bridge 0.19.0; Sofort-Wegsuche ab 0.19.1, direkter Terrain-Reichweitenzugriff ab 0.19.2. Rein lesend; Navigationsdaten können nach Spieländerungen verzögert aktualisiert werden.",
