...
public sealed class NativeTools(NativeClient client) : IDisposable
{
    public static IReadOnlyList<Tool> Catalog()
    {
        ...
        return new[] { "timberborn_status", "inspect_colony", "inspect_map_region", "find_buildings", "inspect_build_catalog", "precheck_build_site" }.Select(name =>
        {
            bool map = name == "inspect_map_region";
            bool site = name == "precheck_build_site";
            ...
            return new Tool { Name = name,
                Description = name == "find_buildings" ? "Liest Gebäude und Wege mit stabilen Vorlagen-IDs, Position, Eingang und bis zu 64 belegten Zellen je Objekt. Maximal 32 Einträge; Seiten sind frische Beobachtungen."
    ...
    public async Task<JsonObject> Invoke(string name, JsonElement args, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (name is "inspect_map_region" or "find_buildings" or "precheck_build_site")
            {
                var query = new NameValueCollection();
                foreach (var property in args.EnumerateObject())
                {
                    if (name == "precheck_build_site" && property.Name == "template" && property.Value.ValueKind == JsonValueKind.String)
