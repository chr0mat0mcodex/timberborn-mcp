...
public sealed class BridgeConfigurator : Configurator
{
    ...
    {
        Bind<BuildingCatalog>().AsSingleton(); Bind<SpatialObservations>().AsSingleton();
    ...
    : ILoadableSingleton, IUnloadableSingleton, IUpdatableSingleton
{
    ...
    public void Load()
    {
        ...
        try
        {
            ...
            server = new BridgeHttpServer((int?)config["port"] ?? 8081, (string?)config["token"] ?? "", queue,
                (bool?)config["enableValidation"] == true, (bool?)config["enablePlacement"] == true, (bool?)config["enableLodgePlacement"] == true, (bool?)config["enableSpeedControl"] == true, (bool?)config["enableStaffing"] == true, (bool?)config["enablePriorities"] == true, (bool?)config["enableAreas"] == true, (bool?)config["enableRemoval"] == true, (bool?)config["enableBuildingPlacement"] == true);
    ...
    {
        if ((request.Route is "building-validation" or "building-placement" or "site-validation" or "path-placement" or "lodge-placement" or "building" or "simulation-speed" or "workplace-staffing" or "priority" or "set-priority" or "set-area" or "remove-object") && request.Session != sessionId) throw new ArgumentException("stale_session");
