...
Beide standardm��ig aus. `inspect_research` bleibt lesend verf�gbar (19 native Leser).

`outcome`: `cost_changed`, `unavailable`, `insufficient_points`, `not_unlockable`
sind Ablehnungen ohne �nderung; `already_unlocked` ist ein unver�nderter Erfolg.
`applied` best�tigt Freischaltung und den erwarteten Punkteabzug auf dem Spielthread.
`unconfirmed` oder ein Transportfehler erfordern ausschlie�lich eine neue Abfrage.
Die Aktion verwendet keine eigenen Save-Daten und ver�ndert keine Baugeometrie.
